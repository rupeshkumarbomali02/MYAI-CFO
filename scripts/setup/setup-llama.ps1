param([string]$Release = "b10516")

$ErrorActionPreference = "Stop"
# Production runtime: release b10516 includes modern Nemotron 3 Nano llama.cpp support.
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$rootDir = Split-Path -Parent (Split-Path -Parent $scriptDir)
$appDir = Join-Path $rootDir "app"
$toolsDir = Join-Path $appDir "tools"
$llmRoot = Join-Path $appDir "llm-backend\win"
$ReleaseMarker = Join-Path $llmRoot '.release'

# Backend-specific marker prevents one installed backend from incorrectly
# certifying every other backend as ready.
function Get-BackendReleaseMarker {
    param([string]$Destination)
    return (Join-Path $Destination '.release')
}
...
function Install-LlamaArchive {
    ...
    $backendMarker = Get-BackendReleaseMarker -Destination $dest
    $alreadyReady = $false
    if ((Test-Path -LiteralPath $server -PathType Leaf) -and (Test-Path -LiteralPath $backendMarker -PathType Leaf)) {
        $markerValue = (Get-Content -LiteralPath $backendMarker -Raw -ErrorAction SilentlyContinue).Trim()
        $alreadyReady = ($markerValue -eq $Release)
    }
    if ($alreadyReady) {
        Write-Host "   OK  llama.cpp $Variant backend already ready ($Release)."
        return $true
    }
    ...
    if (-not (Test-Path $server)) { throw "llama-server.exe was not found after extracting $AssetName" }
    New-Item -ItemType Directory -Force -Path $llmRoot | Out-Null
    Set-Content -LiteralPath $backendMarker -Value $Release -NoNewline
    Write-Host "   OK  llama.cpp $Variant backend installed ($Release)."
    return $true
}
...
function Try-InstallLlamaArchive {
    ...
    try {
        return [bool](Install-LlamaArchive -Variant $Variant -AssetName $AssetName)
    } catch {
        Write-Host "   !!  Skipping llama.cpp $Variant backend: $Reason" -ForegroundColor Yellow
...
if ($hasNvidia) {
    $runtimeReady = Try-InstallLlamaArchive -Variant "cuda" -AssetName "llama-$Release-bin-win-cuda-12.4-x64.zip" -Reason "NVIDIA CUDA acceleration"
}
...
if (-not $runtimeReady -and $hasAmd) {
    $runtimeReady = Try-InstallLlamaArchive -Variant "hip" -AssetName "llama-$Release-bin-win-hip-x64.zip" -Reason "AMD HIP acceleration"
}
...
if (-not $runtimeReady -and $hasIntel) {
    $runtimeReady = Try-InstallLlamaArchive -Variant "sycl" -AssetName "llama-$Release-bin-win-sycl-x64.zip" -Reason "Intel SYCL acceleration"
}

# Vulkan is a useful cross-vendor fallback, but it is not required for first-run
# success. CPU is the final guaranteed fallback. The installer must never abort
# simply because an optional accelerator asset is unavailable.
if (-not $runtimeReady) {
    $runtimeReady = Try-InstallLlamaArchive -Variant "vulkan" -AssetName "llama-$Release-bin-win-vulkan-x64.zip" -Reason "cross-vendor Vulkan fallback"
}
# Always provision a CPU fallback in production. A CUDA/Vulkan backend may be
# installable yet still fail to load a particular GGUF on a specific GPU/runtime.
# Keeping CPU available makes model fallback deterministic instead of leaving the
# entire CFO without an inference backend.
$cpuReady = Try-InstallLlamaArchive -Variant "cpu" -AssetName "llama-$Release-bin-win-cpu-x64.zip" -Reason "universal local inference fallback"
if (-not $runtimeReady -and -not $cpuReady) {
    throw "No usable llama.cpp runtime could be installed. Accelerator and CPU fallback both failed."
}
Write-Host "   OK  llama.cpp runtime ready (accelerator fallback + CPU fallback provisioned)." -ForegroundColor Green
